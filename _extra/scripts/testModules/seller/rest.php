<?php
/**
 * Created by PhpStorm.
 * User: training
 * Date: 11/28/17
 * Time: 4:01 PM
 */

require __DIR__.'/../_tools/init.php';

// Initialize the client
$client = new \SmileCoreTest\RestClient();
$client->setDebug(true);
$client->setMagentoParams($params);
$client->connect();

$object = [
    'object' => [
        'identifier'    => 'second',
        'name'          => 'Second Seller Name'
    ]
];

$search = [
    'searchCriteria' => [
        'filterGroups' => [
            [
                'filters' => [
                    [
                        'field'     => 'identifier',
                        'condition_type' => 'like',
                        'value' => '%on%'
                    ]
                ]
            ]
        ]
    ]
];

$client->post('rest/V1/seller/',$object);
$client->get('rest/V1/seller/identifier/'.$object['object']['identifier']);
$client->delete('rest/V1/seller/identifier/'.$object['object']['identifier']);

$postscriptum = $client->post('rest/V1/seller/',$object);
$client->get('rest/V1/seller/id/'.$postscriptum['seller_id']);
$client->get('rest/V1/seller/?'.http_build_query($search));

$client->delete('rest/V1/seller/id/'.$postscriptum['seller_id']);

